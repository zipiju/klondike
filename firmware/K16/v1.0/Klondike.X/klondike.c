...
BYTE WorkNow, BankSize, ResultQC, HashTime, SlowTick, VerySlowTick, TimeOut, TempTarget, LastTemp, FanLevel;
...
{
    SetAsicClock(DEFAULT_HASHCLOCK);
...
void SetAsicClock(DWORD frequency)
{
    DWORD CLOCK_F_VALUE;
    CLOCK_F_VALUE = (frequency + 6.25) / 12.5;
    Cfg.HashClock = 25 * CLOCK_F_VALUE / 2;
    ClockCfg[0] = 0x20047 | ((CLOCK_F_VALUE & 0x7F) << 21);
    ClockCfg[1] = 0;
